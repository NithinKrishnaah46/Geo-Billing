...
const customers = [
  { id: '1', name: 'ABC Corp', phone: '9876543210' },
  { id: '2', name: 'Tech Solutions', phone: '9876543211' },
  { id: '3', name: 'Global Industries', phone: '9876543212' },
];
...
export function SalesEntryPage() {
  ...
  const [items, setItems] = useState<SalesItem[]>([
    { id: '1', product: '', quantity: 1, price: 0, discount: 0, tax: 18, total: 0 }
  ]);
  ...
  const updateItem = (id: string, field: keyof SalesItem, value: any) => {
    setItems(items.map(item => {
      if (item.id === id) {
        ...
        // Auto-calculate total when price, quantity, discount, or tax changes
        if (field === 'price' || field === 'quantity' || field === 'discount' || field === 'tax') {
  ...
  const totalTax = items.reduce((sum, item) => {
    const afterDiscount = (item.price * item.quantity) - ((item.price * item.quantity * item.discount) / 100);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Sales Entry</h1>
